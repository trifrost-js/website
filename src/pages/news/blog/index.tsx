...
export async function blogRouter <State extends {}> (r:Router<State>) {
	r
			...
			return ctx.html(<Layout title={blog.title} description={blog.desc} section="news">
				<Page style={{paddingTop: css.$v.space_xl}}>
					...
					<p className={css.use('f', 'fa_c', 'fj_c', {
						...
						<a className={css.use('text_body_thin', {color: css.$t.body_fg})} href={blog.author_link} target="_blank">{blog.author_name}</a>
					</p>
					<Article style={{
