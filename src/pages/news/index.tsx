...
function NewsList ({items}:{items: NewsItem[]}) {
	return (<div id="news-list" className={css.use('f', 'fv', {
		...
		>
			<div className={css.use('f', 'fv', 'sp_r_l', {
				...
				{el.desc && <p className={css.use('text_body', {
					lineHeight: 1.4,
					letterSpacing: '.1rem',
					whiteSpace: 'nowrap',
					overflow: 'hidden',
				...
				<div className={css.use('f', 'fh', 'fa_c', 'fj_l', {gap: css.$v.space_s})}>
					<Badge type={el.type} style={{ marginRight: css.$v.space_s }}>{el.type}</Badge>
					<Time date={el.date} format="dddd, MMMM D, YYYY" style={css.mix('text_body_thin', {[css.media.tablet]: {display: 'none'}})} />
...
function Sidebar ({buckets}:{buckets:Record<string, {count:number; label:string}>}) {
	return (<form className={css.use('f', 'fv', 'fg', {gap: css.$v.space_xl})}>
		<fieldset className={css.use('f', 'fv')}>
			<legend className={css.use('text_form_label', {
...
function SidebarWrapper ({buckets}:{buckets:Record<string, {count:number; label:string}>}) {
    return (<div aria-expanded="false" className={css.use('fs0', {
		minWidth: '25rem',
		[css.media.desktop]: {
			position: 'sticky',
    		top: css.$v.space_l,
		...
		<button type="button" className={css.use('button', 'button_t_blue', 'button_s', 'f', 'fa_c', 'fj_c', {
			gap: css.$v.space_xs,
			margin: '0 auto',
		...
		<div className={css.use('f', 'fv', {
			...
			<h1 className={css.use('text_header', {
				[css.media.desktop]: css.mix('sm_b_l'),
				[css.media.tablet]: css.mix('sm_b_xl'),
			})}>Filters</h1>
			<Sidebar buckets={buckets} />
			<button type="button" className={css.use('button', 'button_t_blue', 'button_s', {
				[css.media.desktop]: css.mix('hide'),
			})}>
				Close
				<Script>{el => {
...
export async function newsRouter <State extends {}> (r:Router<State>) {
	r
		.get('', async (ctx) => {
			const list = await NewsService.list(ctx);
			const buckets = buildMonthBuckets(list);

			return ctx.html(<Layout title="TriFrost News" description="A running log of what’s new in TriFrost" section="news">
				<Page width={"130rem"}>
					<div className={css.use('f','fv', 'fa_c', 'sm_h_s', {
						textAlign: 'center',
						...
						<p className={css.use('text_header_thin', {lineHeight: 1.4, maxWidth: '65rem'})}>A running log of what’s new in TriFrost — from releases to behind-the-scenes thoughts</p>
					</div>
					<div className={css.use('f', 'fj_sb', {
						width: '100%',
						gap: css.$v.space_l,
		...
		.post('', async (ctx) => {
			...
			for (let i = 0; i < all.length; i++) {
				const el = all[i];
				if (
					(n_type === null || el.type === n_type) &&
					(n_bucket === null || n_bucket === bucketKey(el.date))
				) list.push(el);
			}
