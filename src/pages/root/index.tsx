...
export async function getHome (ctx:Context) {
	return ctx.html(<Layout section="home">
		<Page style={{position: 'relative'}}>
			<GridBackground />
			<h1 className={css.use('f', 'fa_c', 'fj_c', 'sm_b_l', {
				width: '100%',
			...
			<p className={css.use('text_header_thin', 'sm_h_auto', {
				maxWidth: '65rem',
				textAlign: 'center',
				lineHeight: '140%',
			})}>A blazing-fast, runtime-agnostic server framework built for the modern JavaScript ecosystem — from low-latency edge environments to traditional backend infrastructure</p>
		...
		<div className={css.use('sm_h_auto', {
			...
			<HighLight language="bash" copyEnabled={false} className={css.use({
				backgroundColor: css.$t.panel_bg + ' !important',
				color: css.$t.panel_fg + ' !important',
				border: '1px solid ' + css.$t.panel_border,
			})}>{`npm install @trifrost/core`}</HighLight>
		</div>
	</Layout>);
}

export async function rootRouter <State extends {}> (r:Router<State>) {
	r
		.get('/', getHome)
		...
		.get('/sitemap.xml', async ctx => {
			const sitemap = [
				...
				siteMapEntry('/'),
				...await exampleSitemap(ctx),
				...await NewsService.siteMap(ctx),
				...await DocsService.siteMap(ctx),
			];
			sitemap.push('</urlset>');
