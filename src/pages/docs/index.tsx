...
export function DocsSidebar ({entry}:{entry:Doc}) {
    ...
    return (<aside id="docs-menu" aria-expanded="false" className={mainCls}>
		{DocsService.list().map(section => (<Collapsible
			title={section.title}
			group={collapseCid}
		...
		>
			<ul>
				{section.items.map(item => <li key={item.slug}>
					<a
						href={`/docs/${item.slug}`}
						className={entryCls}
						data-sidebar-entry
...
export function DocsOnThisPage ({tree}:{tree:MarkdownNode[]}) {
    ...
    return (<nav aria-label="Table of contents for article" className={cls}>
		<h2>On this page</h2>
		<div>
			{Markdown.extractHeadersFromNodes(tree).map(el => <a href={`#${el.id}`} key={el.id} data-level={el.level}>{el.title}</a>)}
		</div>
	</nav>);
}

export function DocsMenuMobile ({tree}:{tree:MarkdownNode[]}) {
    return (<nav aria-label="Table of contents for article" aria-expanded="false" className={css.use('f', 'fv', {
		...
		<div className={css.use('f', 'sm_t_s', 'sm_b_l', {gap: css.$v.space_s})}>
			<button type="button" className={css.use('f', 'fh', 'fa_c', 'fj_c', 'button', 'button_s', 'button_t_blue', {
				justifySelf: 'center',
				alignSelf: 'flex-start',
				gap: css.$v.space_s,
				[css.media.desktop]: css.mix('hide'),
...
export async function docsRouter <State extends {}> (r:Router<State>) {
	r
		.get('', ctx => ctx.redirect(DocsService.root().to))
		.get('/:slugId', async ctx => {
			...
			return ctx.html(<Layout section="docs" title={entry.title} description={entry.desc}>
				<Page width={"150rem"}>
					<div className={css.use('f','fv', 'fa_c', 'sm_h_s', {
						...
						<p className={css.use('text_header_thin', {lineHeight: 1.4, maxWidth: '65rem'})}>Learn and build with confidence, from a first project with workers to global scale on bare metal.</p>
					</div>
					<div className={css.use('f', 'fh', 'fg', 'fj_c', {width: '100%'})}>
						<DocsSidebar entry={entry} />
						<div className={css.use('f', 'fh', 'fg', {
							[css.media.tablet]: {width: '100%'}
						})}>
							<div className={css.use('fg', {
								[css.media.desktop]: {maxWidth: 'calc(150rem - 50rem)'},
								[css.media.tablet]: {width: '100%'},
							})}>
								<Article style={css.mix('fs0', {
									[css.media.desktop]: css.mix('sm_h_l', 'fg'),
									[css.media.tablet]: {width: '100%'},
								})}>
									<h2 className={css.use({
