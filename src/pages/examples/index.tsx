      ...
      return (
        <>
          ...
          <div className={css.use('f', 'sm_t_s', 'sm_b_l', {gap: css.$v.space_s})}>
            <Button to="https://htmx-todos.trifrost.dev/" label="View Live" size="s" blank={true} />
            <Button to="/examples/trifrost_htmx_todos/download" label="Download" size="s" />
          </div>
          <p>
            This example showcases a fully interactive, stateful todo application using <strong>TriFrost</strong> and{' '}
            <a href="https://htmx.org/" target="_blank" rel="follow">
              HTMX
            </a>
            .
          </p>
          <h2>How It Works</h2>
          <p>
            This app is entirely server-rendered using TriFrost. Routing, HTML generation, and persistent state are all handled within a
            single <code>index.tsx</code> entrypoint. Todos are stored durably using a <code>DurableObjectCache</code> — so no database
            setup is needed. TriFrost's built-in JSX and CSS support means UI fragments are returned directly from route handlers.
          </p>
          <p>
            On the frontend,{' '}
            <a href="https://htmx.org/" target="_blank" rel="follow">
            ...
            listens for form submissions and button clicks, then sends HTTP requests via attributes like <code>hx-post</code> and{' '}
            <code>hx-delete</code>. The server responds with fragment HTML — e.g. just the updated todo list — and{' '}
            <a href="https://htmx.org/" target="_blank" rel="follow">
              HTMX
            </a>{' '}
            swaps it seamlessly into the page.
          </p>
          <p>
            🧪 Observability is baked in. Every request is traced using OpenTelemetry and exported to{' '}
            <a href="https://uptrace.dev/" target="_blank">
              Uptrace
            </a>
            . App metadata like <code>name</code> and <code>version</code> are included automatically in every span, with no boilerplate
            required. You can inspect the performance and behavior of each route in full detail.
          </p>
          {/* Project Structure */}
          ...
          <Collapsible title="App Logic" tag={'h3'} group={exampleCid}>
            <>
              <p className={css.use('sm_b_l')}>
                The main app logic lives in <code>src/index.tsx</code>, where TriFrost routes, renders, and manages state for the entire
                todo application using a Durable Object–backed cache.
              </p>
...
export default app;`}</HighLight>
          ...
          <Collapsible title="Layout" tag={'h3'} group={exampleCid}>
            <>
              <p className={css.use('sm_b_l')}>
                The layout component wraps full-page responses, injecting meta tags, global styles, the HTMX script, and optional UI
                elements like footers or loading indicators.
              </p>
	...
	return (<html lang="en">
		...
		<body className={css.use(...)}>
          ...
          <Collapsible title="Components" tag={'h3'} group={exampleCid}>
            <>
              <p className={css.use('sm_b_l')}>
                All UI elements like the todo form and todo list live in self-contained components under <code>src/components/</code>. Each
                component renders server-side JSX and uses HTMX attributes to hook into behavior as well as using the TriFrost css instance
                for styling.
              </p>
	...
	return (<section id="todo-list">
		...
		<Style />
          ...
          <Collapsible title="Styling" tag={'h3'} group={exampleCid}>
            <>
              <p className={css.use('sm_b_l')}>
                Instead of a prebuilt css file which gets included everywhere (and of which probably 10% is used per page) the css module
                automatically collects what is used on the page's components and injects it as inline styling.
              </p>
              <p className={css.use('sm_b_l')}>
                It does just a bit more than that, in this particular case we're also injecting a <strong>css reset</strong> as well as
                defining <strong>reusable definitions</strong> and <strong>theme/global variables.</strong>
              </p>
              <p className={css.use('sm_b_l')}>
                An important note on those <strong>definitions</strong> is that they are not included in the css if not used. They form your
                backbone to centralize reusable pieces of styling without bloating the page if not used.
              </p>
	...
	definitions: mod => ({
		...
		form_icon: {
          ...
          <Collapsible title="Environment" tag={'h3'} group={exampleCid}>
            <>
              <p className={css.use('sm_b_l')}>
                All environment bindings expected by the app — such as the durable object, asset fetcher, and optional Uptrace DSN — are
                defined in <code>src/types.ts</code> for type safety and clarity.
              </p>
              <p className={css.use('sm_b_l')}>
                This exported <code>Env</code> type is also used as a generic for the App, this generic ensures each of the routes (as well
                as the app configuration itself) knows the environment.
              </p>
              <p className={css.use('sm_b_l')}>
...
export type Env = {
	...
	UPTRACE_DSN: string; /* DSN from uptrace */
          ...
          <Collapsible title="Cloudflare" tag={'h3'} group={exampleCid}>
            <>
              <p className={css.use('sm_b_l')}>
                The <code>wrangler.toml</code> config defines how your app runs on Cloudflare — including bindings for assets and durable
                objects, compatibility flags, and deployment metadata.
              </p>
...
`}</HighLight>
          ...
          <h2>Screenshots</h2>
          <div
            className={css.use('f', 'fh', 'fj_sb', 'fa_l', 'sp_t_m', 'sm_b_xl', {
          ...
          <LinkList
            list={[
              {label: 'HTMX', to: 'https://htmx.org/', desc: 'Add AJAX, WebSockets, and more to HTML using attributes.'},
              {
                ...
                to: 'https://developers.cloudflare.com/workers/runtime-apis/durable-objects/',
                desc: 'Low-latency stateful storage at the edge.',
              },
              {label: 'UpTrace', to: 'https://uptrace.dev/', desc: 'OTel-powered observability backend used in this example.'},
              {
                label: 'HTML Forms (MDN)',
                to: 'https://developer.mozilla.org/en-US/docs/Web/HTML/Element/form',
                desc: 'Useful refresher on native forms.',
              },
            ]}
          />
        </>
    ...
    body: () => {
      ...
      return (
        <>
          ...
          <div className={css.use('f', 'sm_t_s', 'sm_b_l', {gap: css.$v.space_s})}>
            <Button to="https://website-node-container.trifrost.dev/" label="View Live" size="s" blank={true} />
            <Button to="/examples/trifrost_node_mini_site/download" label="Download" size="s" />
          </div>
          <p>
            This example showcases a multi-page server-rendered <strong>TriFrost</strong> application running on{' '}
            <a href="https://nodejs.org/" target="_blank" rel="follow">
              Node.js
            </a>
            , containerized with{' '}
            <a href="https://podman.io/" target="_blank" rel="follow">
              Podman
            </a>
            . It demonstrates runtime-agnostic rendering,{' '}
            <a href="https://htmx.org/" target="_blank" rel="follow">
              HTMX
            </a>
            -driven interactivity (for comments), and optional observability with{' '}
            <a href="https://signoz.io/" target="_blank" rel="follow">
              SigNoz
            </a>
            .
          </p>
          <h2>How It Works</h2>
          <p>
            The app uses TriFrost’s flexible routing and JSX rendering to deliver a home page, about page, and a blog page with dynamic
            comments. HTMX handles comment posting and deletion via fragment swaps, making it lightweight and responsive without a heavy
            frontend framework.
          </p>
          <p>
            You can run it standalone with Node.js or containerize it with Podman and Podman Compose — a perfect showcase of TriFrost’s
            runtime flexibility (just compare it with our{' '}
            <a href="/examples/trifrost_htmx_todos" target="_blank" rel="follow">
              Cloudflare Example
            </a>{' '}
            and you'll not there's very little that needs to change).
          </p>
          <p>
            This example is set up to optionally send OpenTelemetry traces to{' '}
            <a href="https://signoz.io/" target="_blank" rel="follow">
              SigNoz
            </a>
            , giving you visibility into route performance, request flows, and system health.
          </p>
          <p>
            To enable it, provide your <code>SIGNOZ_API_TOKEN</code> in a <code>.env</code> file and uncomment the{' '}
            <code>OtelHttpExporter</code> block in <code>index.ts</code> and you're good to go.
          </p>
...
└─ .env`}</HighLight>
          ...
          <Collapsible title="App Logic" tag={'h3'} group={exampleCid}>
            <>
              <p>
                The app is initialized in <code>index.ts</code> using TriFrost’s <code>App</code> class. Middleware like security and CORS
                are added, then routing groups (<code>homeRouter</code>, <code>aboutRouter</code>, <code>blogRouter</code>) are wired in.
                Each page uses server-rendered JSX; the blog section includes dynamic comment handling with HTMX.
              </p>
...
new App<Env>({
    ...
    .boot({port: Number(process.env.PORT || 3000)});`}</HighLight>
          ...
          <Collapsible title="Page Breakdown: Home, About, Blog" tag={'h3'} group={exampleCid}>
            <>
              <p>
                Each of these pages is set up as its own router group (<code>homeRouter</code>, <code>aboutRouter</code>,{' '}
                <code>blogRouter</code>) and connected in the main <code>index.ts</code>. They use a shared layout wrapper for consistency,
                pulling in navigation, headers, and theme-aware styles.
              </p>
              <ul className={css.use('sm_t_l')}>
                <li>
                  ...
                  → Provides a welcoming introduction to TriFrost, explaining its mission as a runtime-agnostic, fast server framework. Uses
                  a simple JSX layout with hero text and buttons.
                </li>
                <li>
                  <strong>
                    About Page (<code>/about</code>)
                  </strong>{' '}
                  → Gives background on what makes TriFrost special — its composable middleware, multi-runtime compatibility (Node, Bun,
                  Workers), and built-in observability. This page uses bullet lists and styled sections.
                </li>
                <li>
                  <strong>
                    Blog Page (<code>/blog</code>)
                  </strong>{' '}
                  → The most interactive section, listing blog posts and allowing users to submit and delete comments live via HTMX. This
                  showcases TriFrost’s ability to handle fragment rendering and dynamic state updates without a heavy frontend framework.
                </li>
              </ul>
            </>
          </Collapsible>
          <Collapsible title="CSS/Theming" tag={'h3'} group={exampleCid}>
            <>
              <p>
                Styling is centralized in <code>css.ts</code> using TriFrost’s <code>createCss</code> system. It defines dark/light themes,
                font families, spacing scales, and responsive helpers, enabling consistent component styling and easy overrides.
              </p>
              <p className={css.use('sm_t_l')}>
                Once a var/theme variable is defined they are respectively available at <code>css.$v.[variable name]</code> and{' '}
                <code>css.$t.[variable name]</code> anywhere in your app.
              </p>
...
document.documentElement.setAttribute('data-theme', 'light'); // Switch to light mode`}</HighLight>
          ...
          <Collapsible title="Containerization" tag={'h3'} group={exampleCid}>
            <>
              <p>
                The <code>Containerfile</code> uses a multi-stage build: first compiling the TypeScript project, then packaging only
                production files.
              </p>
              <p className={css.use('sm_t_l')}>This is also the file that builds your source into a container ready for deployment.</p>
...
volumes:
  node_modules:`}</HighLight>
          ...
          <Collapsible title="Environment" tag={'h3'} group={exampleCid}>
            <>
              <p>
                We define an <code>Env</code> type in <code>types.ts</code> to describe environment bindings like <code>PORT</code> and
                optional observability tokens like <code>SIGNOZ_API_TOKEN</code>. This lets the app pull runtime config from{' '}
                <code>process.env</code> without hardcoding values, making it deployment-flexible.
              </p>
              <p className={css.use('sm_t_l')}>
                We also define our own app-specific <code>Context</code> and <code>Router</code> types here. These will automatically be
                aware of our Environment simply by passing it as a generic.
              </p>
              <p className={css.use('sm_t_l')}>
                Important to note, when working with <strong>Podman</strong>, you will need to also provide the Environment bindings in{' '}
                <code>compose.yml</code> for local development.
              </p>
              <HighLight language="ts">{`// src/types.ts
