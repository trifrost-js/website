...
export class Markdown {
    ...
    static extractTextFromNodes (nodes: MarkdownNode[]): string {
        return join(nodes.flatMap(node => {
            switch (node.type) {
                case 'text':
                case 'inlineCode':
                    return node.content;
        ...
        return tree.map((node, i) => {
            switch (node.type) {
                case 'heading': {
                    ...
                    return <H id={Markdown.slugify(text)} key={i}>{Markdown.renderTree(node.children)}</H>;
                }
                case 'paragraph':
                ...
                case 'codeBlock':
				  	return <HighLight key={i} language={node.language}>{node.code}</HighLight>;
                case 'list':
				  	return <ul key={i}>
                        {node.children.map((child, j) => <li key={j}>{Markdown.renderTree(child.children)}</li>
					  	)}
                    </ul>;
                case 'listItem':
                ...
                case 'link':
                    return <a key={i} href={node.href}>{Markdown.renderTree(node.children)}</a>;
                case 'image':
                    return <Image src={node.src} alt={node.alt || ''} />;
                case 'runtimeWrapper':
                    return <Runtime key={i} runtimes={node.runtimes}>
                        {node.children.map((block, j) => <RuntimeBlock key={j} runtime={block.runtime}>
