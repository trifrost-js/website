import {Style, Script} from '@trifrost/core/modules/JSX';
...
export function Root (props:{
	...
	const title = isNeString(props.title)
		? props.title
		: 'TriFrost';
	const desc = isNeString(props.description)
		? props.description
		: 'Blazing-fast, runtime-agnostic server framework for modern JavaScript runtimes';
	const pageTitle = title === 'TriFrost' ? 'TriFrost' : `TriFrost | ${title.replace(/^TriFrost /, '')}`;

	return (<html lang="en" className={css.use({
		width: '100%',
		height: '100%',
		fontSize: '62.5%',
		...
		<body className={css.use('f', 'fv', 'fa_c', 'fj_sb', {
			width: '100%',
			height: '100%',
			overflowX: 'hidden',
			lineHeight: 1,
