...
export function RuntimeBlock ({runtime, children, ...rest}:RuntimeBlockOptions) {
    return <div data-runtime={runtime} {...rest}>{children}</div>;
}
...
export function Runtime ({runtimes, children, ...rest}:RuntimeOptions) {
    ...
    return <div className={`runtime ${css.use('panel_alt', 'sp_s', 'br_m', cssBlock)}`} data-active-runtime={runtimes[0]} {...rest}>
        <div className={css.use('f', 'fh', 'fa_c', 'fj_l', 'sm_b_s', {gap: css.$v.space_s})}>
            {runtimes.map(runtime => <button
                type="button"
                key={runtime}
                data-runtime={runtime}
            >{runtime.toUpperCase()}</button>)}
        </div>
        <div>{children}</div>
