...
function HeaderTrigger() {
  ...
  return (
    <button type="button" className={cls} aria-hidden="true">
      <span></span>
      <span></span>
      <span></span>
      <Script>
...
function HeaderItem(props: {key: string; active: string; el: (typeof list)[number]}) {
  const cls = css.use('br_l', {
    ...
    cursor: 'pointer',
    color: css.$t.body_fg,
    [[`${css.hover}${css.not(css.attr('data-disabled'))}`, css.attr('data-active')].join(',')]: {
  ...
  return props.el.to ? (
    <a href={props.el.to} key={props.key} rel="follow" className={cls} {...(props.active === props.el.name && {'data-active': true})}>
      {props.el.label}
    </a>
  ) : (
    <span className={cls} data-disabled>
      {props.el.label}
    </span>
  );
}
...
function HeaderIcon(props: {key: string | number; el: (typeof icons)[number]}) {
  ...
  return (
    <a className={cls} href={to} key={props.key} title={title} target="_blank" rel="follow">
      <Icon width={24} />
    </a>
  );
}
...
export function Header({active, ...rest}: HeaderProps) {
  ...
  return (
    <header className={cls} {...rest}>
      ...
      >
        ...
        >
          {list.map(el => (
            <HeaderItem el={el} key={el.name} active={active} />
          ))}
        </div>
        <div id="docsearch"></div>
        <div className={css.use('fg', {[css.media.desktop]: css.mix('hide')})}></div>
        <aside
          className={css.use('f', 'fg', 'fj_r', 'fa_c', {
